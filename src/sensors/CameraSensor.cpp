...
#include <mars_utils/Geometry.hpp>
...
namespace mars
{
    namespace core
    {
            ...
            //logicalCamera(id,name,config.width,config.height,1,false, true)
        {
            ...
            // todo: how to correctly get the draw_id
            draw_id = this->config.map["draw_id"];
        ...
        void CameraSensor::getDepthImage(std::vector<DistanceMeasurement>& buffer) const
        {
            assert(buffer.size() == (config.width * config.height));
            int width;
            int height;
         ...
         */
        // todo: need some clever reimplementation?
            ...
            for(unsigned int y=0; y<height; ++y)
            {
                for(unsigned int x=0; x<width; ++x)
                {
                    ...
                    if((depth <= maxDistance) and !std::isnan(depth))
                    {
                        ...
                        colors->push_back(base::Vector4d(buffer[index2]*s,
                                                         buffer[index2+1]*s,
                                                         buffer[index2+2]*s,
                                                         1.0));
                    }
        ...
        void CameraSensor::postGraphicsUpdate(void)
        {
            if(gc && renderCam == 1)
            {
                ...
                //Quaternion qcorrect = Quaternion(0.5, 0.5, -0.5, -0.5);
                //orientation = control->graphics->getDrawObjectQuaternion(draw_id) * config.ori_offset * qcorrect;
                position += (control->graphics->getDrawObjectQuaternion(draw_id) * config.pos_offset);
            }
        }
                                              ...
                                              ConfigMap *config)
        {
            ...
            if((it = config->find("frame_skip")) != config->end())
                cfg->frameSkip = it->second;

            if(cfg->frameOffset == -1)
            {
                cfg->frameOffset = CameraConfigStruct::globalFrameOffset++;
            }
...
//      cfg["enabled"] = config.enabled;
        ...
        bool CameraSensor::haveNewData()
        {
            return have_new_data;
        }

        void CameraSensor::getPose(utils::Vector &p, utils::Quaternion &q)
        {
            p = position;
            q = orientation;
        }
